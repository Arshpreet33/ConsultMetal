...
function runCheck() {
  ...
  const varsPath = path.join(root, "src", "assets", "styles", "variables.css");
  if (!fs.existsSync(varsPath)) {
    console.error("Could not find variables.css at", varsPath);
    return;
  }
  const vars = readVariables(varsPath);
  const files = walkDir(path.join(root, "src")).filter((f) =>
    f.endsWith(".css")
  );
  const pairs = [];
  files.forEach((f) => {
    const r = scanCssForPairs(f, vars);
    r.forEach((rr) => pairs.push(rr));
  });
  const report = [];
  // Check common variable pairs as well
  const pairsToCheck = [
    ["text-primary", "bg-primary"],
    ["text-secondary", "bg-primary"],
    ["text-white", "color-primary"],
    ["text-white", "color-primary-dark"],
    ["color-primary", "text-white"],
  ];
  pairsToCheck.forEach(([txt, bg]) => {
    const txtVal = vars[txt];
    const bgVal = vars[bg];
    if (txtVal && bgVal) {
      report.push({
        selector: `var(${txt}) on var(${bg})`,
        file: "variables",
        color: txtVal,
        background: bgVal,
        ratio: contrastRatio(toRgb(txtVal), toRgb(bgVal)),
      });
    }
  });
  pairs.forEach((p) => {
    const textColor = toRgb(p.color);
    const bgColor = toRgb(p.background);
  ...
  const failing = report.filter((r) => r.ratio < 4.5);
  const warnings = report.filter((r) => r.ratio >= 4.5 && r.ratio < 7);
  ...
  if (failing.length > 0) {
    ...
    failing.forEach((f) => {
      console.log(
        `- ${f.file} ${f.selector} : ${f.color} on ${f.background} = ${Number(
          f.ratio
        ).toFixed(2)}`
      );
    });
